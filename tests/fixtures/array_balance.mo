...
model SimpleStateSpace
  ...
  parameter Real A[n,n] = {{-1, 0}, {0, -2}};
  parameter Real B[n,1] = {{1}, {1}};
  parameter Real C[p,n] = {{1, 1}};
  parameter Real D[p,1] = {{0}};
  input Real u;
  Real x[n](each start = 0);
  output Real y[p];
equation
...
model SimpleTransferFunction
  ...
  parameter Real b[n+1] = {1, 0, 0} "Numerator coefficients";
  parameter Real a[n+1] = {1, 2, 1} "Denominator coefficients";
  input Real u;
  output Real y;
  Real x[n](each start = 0);
equation
